...
column_list* valid_columns(char* current_query, char* table_name, char* key_column_1, char* key_column_2, char* value_column_1, bool *validity){
    ...
    if(!search_columns(table_name, key_column_1, key_column_2, value_column_1, HEAD)){
        *validity = false;
    }
    *validity = true;
    return HEAD;
...
Datum metadata_handler(PG_FUNCTION_ARGS){
    ...
    bool validity = false;
    column_list* HEAD = valid_columns(current_query, table_name, key_column_1, key_column_2, value_column_1, &validity);
    if(!(validity)){
        PG_RETURN_NULL();
    }
